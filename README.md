...
Requisições sem o header ou com uma chave inválida retornam os seguintes erros:

| HTTP | Código de erro      | Descrição                     |
|------|---------------------|-------------------------------|
| 401  | `MISSING_API_KEY`   | Header `X-Api-Key` não enviado |
...
Endpoints de busca por ID retornam:
...
|-----------------------|------|----------------------------------------------------|
| `MISSING_API_KEY`     | 401  | Header de autenticação ausente                     |
| `INVALID_API_KEY`     | 401  | API Key inválida ou revogada                       |
| `VALIDATION_ERROR`    | 400  | Parâmetro inválido ou ausente                      |
| `DATE_RANGE_EXCEEDED` | 400  | Período de consulta excede 366 dias                |
| `NOT_FOUND`           | 404  | Recurso não encontrado ou não pertence à agência   |
...
Retorna todas as contas bancárias ativas da agência.

**Não possui filtros de query.** A resposta inclui todas as contas sem paginação.

#### Exemplo de requisição

```bash
curl -X GET "https://api.iclips.com.br/api/v1/financial/accounts" \
...
| `to`             | date    | **Sim**     | Data de fim do período (`YYYY-MM-DD`). Máximo de 366 dias de diferença entre `from` e `to`.        |
| `type`           | string  | Não         | Tipo do lançamento. Valores: `Entrada`, `Saída`, `A Receber`, `A Pagar`                            |
| `account_id`     | integer | Não         | Filtra pelo ID da conta bancária (use `/accounts` para obter os IDs)                               |
| `category_id`    | integer | Não         | Filtra pelo ID da categoria do lançamento                                                          |
| `dre_indicator`  | string  | Não         | Filtra pelo indicador DRE da categoria pai. Ver tabela de valores abaixo.                          |
...
| `limit`          | integer | Não         | Quantidade de registros por página. Padrão: `50`. Mín: `1`. Máx: `200`.                            |
...
| `NENHUM`                                 | Categoria sem indicador DRE definido   |
| `OUTRO`                                  | Demais categorias                      |

#### Exemplo de requisição

```bash
curl -X GET "https://api.iclips.com.br/api/v1/financial/entries?from=2025-01-01&to=2025-03-31&type=Sa%C3%ADda&limit=20&offset=0" \
  -H "X-Api-Key: sk_live_a1b2c3d4e5f6"
```
...
| `parentEntryId`            | integer         | ID do lançamento pai (para parcelamentos). `0` se não houver                       |
...
| `entryDate`                | datetime / null | Data de competência/realização do lançamento                                       |
...
| `invoice.number`           | string / null   | Número da nota fiscal vinculada                                                    |
...
| `costCenters`              | array           | Lista de centros de custo rateados no lançamento                                   |
...
```bash
curl -X GET "https://api.iclips.com.br/api/v1/financial/entries/4801" \
  -H "X-Api-Key: sk_live_a1b2c3d4e5f6"
```
...
> Os campos da resposta são idênticos aos descritos no endpoint de listagem. Consulte a [tabela de campos acima](#campos-da-resposta-1).
...
| `to`      | date    | **Sim**     | Data de fim do período (`YYYY-MM-DD`). Máximo de 366 dias de diferença entre `from` e `to`.   |
| `status`  | string  | Não         | Status da nota fiscal. Valores: `pending`, `issued`, `cancelled`, `error`                     |
| `limit`   | integer | Não         | Quantidade de registros por página. Padrão: `50`. Mín: `1`. Máx: `200`.                        |
| `offset`  | integer | Não         | Deslocamento para paginação. Padrão: `0`.                                                      |
...
```bash
curl -X GET "https://api.iclips.com.br/api/v1/financial/invoices?from=2025-01-01&to=2025-03-31&status=issued&limit=10&offset=0" \
  -H "X-Api-Key: sk_live_a1b2c3d4e5f6"
```
...
| `issRetained`                  | boolean         | Indica se o ISS será retido na fonte pelo tomador                           |
...
```bash
curl -X GET "https://api.iclips.com.br/api/v1/financial/invoices/310" \
  -H "X-Api-Key: sk_live_a1b2c3d4e5f6"
```
...
> Os campos da resposta são idênticos aos descritos no endpoint de listagem. Consulte a [tabela de campos acima](#campos-da-resposta-3).
...
```bash
# Página 1
curl "https://api.iclips.com.br/api/v1/financial/entries?from=2025-01-01&to=2025-03-31&limit=50&offset=0" \
  -H "X-Api-Key: sk_live_a1b2c3d4e5f6"

# Página 2
curl "https://api.iclips.com.br/api/v1/financial/entries?from=2025-01-01&to=2025-03-31&limit=50&offset=50" \
  -H "X-Api-Key: sk_live_a1b2c3d4e5f6"

# Página 3
curl "https://api.iclips.com.br/api/v1/financial/entries?from=2025-01-01&to=2025-03-31&limit=50&offset=100" \
  -H "X-Api-Key: sk_live_a1b2c3d4e5f6"
```
